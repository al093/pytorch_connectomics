...
def collate_fn_2(batch):
    ...
    :return:
    """
    pos, out_input, out_label_input, out_label, weights, weight_factor = zip(*batch)
    out_input = torch.stack(out_input, 0)
    out_label = torch.stack(out_label, 0)
    out_label_input = torch.stack(out_label_input, 0)
    weights = torch.stack(weights, 0)
    weight_factor = np.stack(weight_factor, 0)
    return pos, out_input, out_label_input, out_label, weights, weight_factor
